...
<div class="col-lg-8 formcontainer bgwhite">
    Если сообщение: 
        ...
        <select name="speed" id="speed">
          <option>в точности совпадает с одним из ключевых слов</option>
          <option>содержит одно из ключевых слов</option>
    ...
    <?php ActiveForm::begin(["id"=>"actionform","options"=>["style"=>"margin-top:22px;"]])?>
        <div class="form-group">
            ...
            <textarea class="form-control answerbox hide" name="command"><?=$com->command?></textarea>
            <div class="form-control answerbox" style="margin-top:20px;" >
                <div contenteditable>
					<?=$com->response?>
                </div>
            <!-- Панель блока ответа с макросами и управлением -->
                <div class="editbox-control" >
                    ...
                    <button type=button data-toggle="modal" class="rules" data-target="#myModalCommand">
            ...
            <textarea class="form-control answerbox hide" name="response"><?=($com?$com->response:"")?></textarea>
...
<div class="col-lg-4 leftblock">
    ...
    <div style="margin-top:35px;">
        <button class="btn btnvk savebtn" type="submit" onclick="console.log($('.answerbox>div:first-of-type').parent().parent().find('textarea'));$('.answerbox>div:first-of-type').parent().parent().find('textarea').val($('.answerbox>div:first-of-type').html());alert();$('#actionform').submit();">Сохранить</button>
    ...
    <div class="panel-vk">
        <b>Команда <b class="enabledaction"><?=($com->enabled?"включена":" выключена")?></b>
        <div class="toggle-button-cover">
              <div class="button-cover">
                <div class="button r" id="button-1">
                  <input type="checkbox" class="checkbox"  onchange="$.get('/bot/action?id=5&action=3&enabled='+(this.value));if($('.enabledaction').html()=='включена')$('.enabledaction').html('выключена');else $('.enabledaction').html('включена');" <?=($com->enabled?"checked=checked":"")?>>
                  <div class="knobs"></div>
                  <div class="layer"></div>
                </div>
              </div>
        ...
        <span>Команда использовалась: <?=($com->uses?$com->uses:0)?> раз</span>
    </div>
    <div class="row">
        <div class="col-lg-12">
            <button class="btnvkwhite col-lg-12 keyboard" data-toggle="modal" data-target="#myModal">
        ...
        <div class="col-md-4">
            <button class="btnvkwhite col-lg-12 vars" data-toggle="modal" data-target="#myModalVars">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<div class="col-lg-8 bgwhite" style="margin-top:20px;">
    ...
    <div>
        ...
        <div class="form-group" style="margin-top:30px;">
            ...
            <div class="form-control answerbox" contenteditable><?=($com?$com->command:"")?>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
          <?php if ($tariff->modules->keyboard == '0') : ?>
            <div class="alert alert-danger">
                ...
                <br>
                <a class="uppertext" href="<?= Url::to(['/pricelist']) ?>">Купить подписку</a>
            </div>
        <?php else: ?>
            <?php Pjax::begin() ?>
                <div class="keyboard-constructor m-1">
...
<!-- VarsModal -->
<div id="myModalVars" class="modal fade" role="dialog">
  <div class="modal-dialog" style="width:max-content; height:max-content">  
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Переменные</h4>
      </div>
       <?php if ($tariff->modules->vars): ?>
      <?php Pjax::begin() ?>
  
        <div class="vars-constructor m-1">
      ...
      <div class="modal-body">
         <div class="alert alert-danger">
            ...
            Создание переменных работает только для сообществ с тарифом "Начальный" или выше.
            <br>
                <a class="uppertext" href="<?= Url::to(['/pricelist']) ?>">Купить подписку</a>
        </div>
...
<!-- CommandModal -->
<div id="myModalCommand" class="modal fade" role="dialog">
  <div class="modal-dialog" style="width:max-content; height:max-content">  

    <!-- CommandModal content-->
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      ...
      <div class="modal-body">
      <?php if ($tariff->modules->rules): ?>

        <?php Pjax::begin() ?>
        <div class="rules-constructor">
        </div>
        <?php Pjax::end() ?>
    <?php else: ?>
            <div class="alert alert-danger">
                ...
                Условия отправки работают только для сообществ с тарифом "Начальный" или выше.
                <br>
                <a class="uppertext" href="<?= Url::to(['/pricelist']) ?>">Купить подписку</a>
            </div>
    <?php endif; ?>
...
$(".keyboard").click(
    function () {
    $.get("/bot/keyboardconstructor",{id:command_id}).done(function(data){
        $(".keyboard-constructor").html(data);
});});

$(".vars").click(function () {
    $.get("/bot/addvar",{command_id:command_id, bot_id:bot_id}).done(function(data){
        $(".vars-constructor").html(data);
    });
});
$(".rules").click(function () {
    $.get("/bot/setrule",{command_id:command_id, bot_id:bot_id}).done(function(data){
        $(".rules-constructor").html(data);
    });
});
JS;
$this->registerJs(
