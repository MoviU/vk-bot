<?php

/* @var $this yii\web\View */
/* @var $form yii\bootstrap\ActiveForm */
/* @var $model app\models\LoginForm */

use yii\helpers\Url;
use yii\helpers\Html;
use yii\bootstrap\ActiveForm;

$this->title = 'Вход в аккаунт';

?>
<div class="row">
<div class="site-login col-lg-6 col-md-7">
    <div class="block-header">

    <svg width="42" height="50" style="float:left;" viewBox="0 0 42 50" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
<rect width="41.6667" height="50" fill="url(#pattern0)"/>
<defs>
<pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
<use xlink:href="#image0" transform="scale(0.0142857 0.0119048)"/>
</pattern>
<image id="image0" width="70" height="84" xlink:href="data:image/png;base64,"/>
</defs>
</svg>
<span class="logo-text"><?=yii::$app->name;?></span>



<a href="<?=Url::to(["site/register"]);?>" class="pull-right" style="color:#000000;opacity:0.5">
    
    <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M17.5 14C20.0783 14 22.1666 11.9117 22.1666 9.33334C22.1666 6.755 20.0783 4.66667 17.5 4.66667C14.9216 4.66667 12.8333 6.755 12.8333 9.33334C12.8333 11.9117 14.9216 14 17.5 14ZM6.99996 11.6667V8.16667H4.66663V11.6667H1.16663V14H4.66663V17.5H6.99996V14H10.5V11.6667H6.99996ZM17.5 16.3333C14.385 16.3333 8.16663 17.8967 8.16663 21V23.3333H26.8333V21C26.8333 17.8967 20.615 16.3333 17.5 16.3333Z" fill="#24282D"/>
</svg>

    
     Регистрация</a>
<a href="<?=Url::to(["site/login"]);?>" class="pull-right">
    
    <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M10.5 19.8333L14 16.4033C13.545 16.3567 13.2067 16.3333 12.8333 16.3333C9.71833 16.3333 3.5 17.8967 3.5 21V23.3333H14L10.5 19.8333ZM12.8333 14C15.4117 14 17.5 11.9117 17.5 9.33334C17.5 6.755 15.4117 4.66667 12.8333 4.66667C10.255 4.66667 8.16667 6.755 8.16667 9.33334C8.16667 11.9117 10.255 14 12.8333 14Z" fill="#4B77A6"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M18.0483 23.9167L14 19.8333L15.6333 18.1883L18.0483 20.615L24.0333 14.5833L25.6667 16.2283L18.0483 23.9167Z" fill="#4B77A6"/>
</svg>


     Вход</a>
     <div class="clearfix"></div>
</div>
    <hr >
    <center>Войдите с помощью вашего аккаунта
    <br>
     <?php  $authch = yii\authclient\widgets\AuthChoice::begin([
     'baseAuthUrl' => ['site/auth'],
     'popupMode' => false,
    "autoRender"=>false
]) ;
foreach($authch->getClients() as $client){

?>
    
    <button class="authvk" onclick="window.location.href='<?=Url::to(['site/auth', 'authclient'=> $client->name])?>'">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0)">
            <path d="M23.8728 17.5199C23.8437 17.4573 23.8166 17.4053 23.7915 17.3637C23.3751 16.6138 22.5793 15.6933 21.4047 14.6019L21.3799 14.5769L21.3675 14.5647L21.3549 14.5521H21.3423C20.8093 14.0439 20.4717 13.7023 20.3303 13.5273C20.0716 13.1941 20.0137 12.8568 20.1551 12.515C20.2549 12.2568 20.6301 11.7114 21.2797 10.8782C21.6214 10.4367 21.892 10.0828 22.0918 9.81614C23.5331 7.90002 24.158 6.67559 23.9663 6.14238L23.8919 6.01777C23.8418 5.94276 23.7127 5.87413 23.5047 5.81154C23.2962 5.74909 23.0297 5.73876 22.7047 5.78038L19.106 5.80524C19.0477 5.78458 18.9645 5.78651 18.856 5.81154C18.7477 5.83658 18.6935 5.84914 18.6935 5.84914L18.6309 5.88043L18.5812 5.91803C18.5395 5.94289 18.4937 5.98661 18.4436 6.04911C18.3939 6.11139 18.3523 6.18448 18.319 6.26777C17.9272 7.27577 17.4817 8.21295 16.9818 9.07928C16.6735 9.59586 16.3904 10.0436 16.1319 10.4226C15.8737 10.8016 15.6571 11.0807 15.4823 11.2597C15.3073 11.4388 15.1493 11.5824 15.0074 11.6908C14.8657 11.7992 14.7575 11.845 14.6827 11.8283C14.6076 11.8115 14.5369 11.7949 14.47 11.7783C14.3534 11.7032 14.2597 11.6012 14.189 11.4721C14.118 11.343 14.0702 11.1805 14.0452 10.9847C14.0204 10.7888 14.0057 10.6203 14.0015 10.4786C13.9976 10.3371 13.9994 10.137 14.0078 9.87877C14.0165 9.62041 14.0204 9.44561 14.0204 9.35396C14.0204 9.03735 14.0265 8.69373 14.0389 8.32303C14.0515 7.95232 14.0617 7.6586 14.0703 7.44221C14.0788 7.22561 14.0827 6.99645 14.0827 6.75485C14.0827 6.51326 14.068 6.32379 14.0389 6.18627C14.0102 6.04893 13.9661 5.91562 13.9081 5.78638C13.8496 5.65726 13.7641 5.55739 13.6519 5.48648C13.5395 5.41567 13.3997 5.35947 13.2333 5.31772C12.7917 5.2178 12.2295 5.16375 11.5462 5.15534C9.99683 5.13871 9.00126 5.23876 8.55974 5.45537C8.3848 5.54688 8.22649 5.67192 8.08495 5.8301C7.93496 6.01344 7.91404 6.11349 8.02232 6.12995C8.52223 6.20483 8.87613 6.38397 9.08442 6.66719L9.15948 6.81726C9.21786 6.92554 9.27616 7.11724 9.3345 7.3921C9.39275 7.66696 9.43035 7.97101 9.44689 8.30407C9.48847 8.9123 9.48847 9.43296 9.44689 9.86608C9.40518 10.2994 9.36579 10.6367 9.3282 10.8783C9.2906 11.1199 9.23441 11.3156 9.15948 11.4655C9.08442 11.6155 9.03448 11.7071 9.00944 11.7404C8.98445 11.7737 8.96362 11.7946 8.94708 11.8029C8.8388 11.8443 8.72618 11.8655 8.60963 11.8655C8.4929 11.8655 8.35136 11.8071 8.18478 11.6904C8.01825 11.5737 7.84542 11.4135 7.66628 11.2093C7.48714 11.0052 7.28511 10.7198 7.0601 10.3533C6.83527 9.98683 6.60199 9.55367 6.3604 9.05385L6.16052 8.69137C6.03556 8.45818 5.86487 8.11863 5.64827 7.67304C5.43153 7.22727 5.23996 6.79608 5.07343 6.37955C5.00686 6.20461 4.90685 6.07143 4.77358 5.97978L4.71104 5.94219C4.66946 5.90892 4.60272 5.8736 4.51116 5.83596C4.41946 5.79837 4.32379 5.77141 4.22374 5.75482L0.799886 5.77968C0.450013 5.77968 0.21262 5.85894 0.0876216 6.01725L0.0375959 6.09213C0.0126049 6.13384 0 6.20046 0 6.29215C0 6.3838 0.024991 6.49628 0.0750167 6.62946C0.574836 7.80417 1.11838 8.93708 1.70564 10.0284C2.29291 11.1197 2.80323 11.9987 3.23631 12.6648C3.66947 13.3313 4.11099 13.9604 4.56087 14.5516C5.01076 15.1432 5.30855 15.5222 5.45425 15.6888C5.60012 15.8556 5.7147 15.9803 5.79799 16.0636L6.11045 16.3635C6.31037 16.5634 6.60396 16.8029 6.99135 17.0819C7.37882 17.3611 7.80778 17.636 8.27845 17.907C8.7492 18.1775 9.29686 18.3982 9.92172 18.569C10.5465 18.7399 11.1546 18.8085 11.7461 18.7754H13.1832C13.4746 18.7502 13.6954 18.6586 13.8455 18.5004L13.8952 18.4378C13.9286 18.388 13.9599 18.3108 13.9888 18.2068C14.0181 18.1027 14.0326 17.988 14.0326 17.8632C14.0241 17.5051 14.0513 17.1823 14.1136 16.8949C14.1759 16.6076 14.2469 16.391 14.3263 16.2452C14.4057 16.0994 14.4952 15.9764 14.5949 15.8767C14.6947 15.7768 14.7659 15.7163 14.8077 15.6955C14.8491 15.6745 14.8823 15.6603 14.9073 15.6517C15.1072 15.5851 15.3425 15.6496 15.6135 15.8456C15.8843 16.0413 16.1383 16.2831 16.3758 16.5704C16.6133 16.8579 16.8985 17.1805 17.2317 17.5387C17.5651 17.8969 17.8566 18.1633 18.1064 18.3385L18.3562 18.4884C18.523 18.5885 18.7396 18.6801 19.0063 18.7634C19.2724 18.8466 19.5056 18.8675 19.7058 18.8258L22.9046 18.7759C23.221 18.7759 23.4672 18.7236 23.6419 18.6196C23.8168 18.5155 23.9208 18.4008 23.9543 18.276C23.9877 18.1511 23.9895 18.0094 23.9607 17.8509C23.931 17.6929 23.9018 17.5824 23.8728 17.5199Z" fill="#4B77A6"/>
            </g>
            <defs>
            <clipPath id="clip0">
            <rect width="24" height="24" fill="white"/>
            </clipPath>
            </defs>
            
        </svg>
        <span class="pull-right">Вконтакте</span>
    </button>
    <?php } yii\authclient\widgets\AuthChoice::end();?>
    </center>
    
    <div class="delimeter">
        <div class=""></div>
        <div class="orlabel">или</div>
        
    
    </div>
    
    
    <?php $form = ActiveForm::begin([
        'id' => 'login-form',
        'layout' => 'horizontal',
        
        'fieldConfig' => [
            'template' => "{label}\n<div class=\"col-lg-12\">{input}</div>\n<div class=\"col-lg-12\">{error}</div>",
            'labelOptions' => ['class' => 'col-lg-12 control-label'],
        ],
    ]); ?>

        <?= $form->field($model, 'email')->textInput(['autofocus' => true])->label("")->textInput(["placeholder"=>"Email"]) ?>

        <?= $form->field($model, 'password')->passwordInput()->label("")->passwordInput(["placeholder"=>"Пароль"]) ?>

        <div class="form-group">
            <div class=" col-lg-12">
                <?= Html::submitButton('Войти', ['class' => 'btn authsend', 'name' => 'login-button']) ?>
            </div>
        </div>

    <?php ActiveForm::end(); ?>
  
    <div class="col-lg-offset-1" style="color:#999;">
      
    </div>
</div>
</div>
